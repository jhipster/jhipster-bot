    ...
    this.pool.connect(function(err, client, done) {
      ...
      client.query('INSERT INTO git_user (name, email, token) values($1, $2, $3)',
        [user.name, user.email, user.token], function(err, result) {
            ...
            results = result.rows[0]; //TODO check the return value
            callback(error, results);
      });
    });
};
...
UserDAOPostgre.prototype.delete = function(id, callback){
    var results = {};
    var error;
    ...
    this.pool.connect(function(err, client, done) {
      ...
      client.query('DELETE FROM git_user WHERE id = $1;', [id], function(err, result) {
        done();
        if(err) {
          callback(err, results);
          return;
...
function checkUser(user){
    ...
    if(!user.name || !user.email || !user.token){
        msgError = 'Error with User object. Lacking attributes: '
        if(!user.name){
            msgError = msgError + 'name,';
        }
        if(!user.email){
            msgError = msgError + 'email,';
        }
        if(!user.token){
            msgError = msgError + 'token,';
        }
    }
    return msgError;
};
