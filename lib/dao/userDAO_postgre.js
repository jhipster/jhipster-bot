...
UserDAOPostgre.prototype.create = function(user, callback){
    ...
    this.pool.connect(function(err, client, done) {
      ...
      client.query('INSERT INTO git_user (name, token) values($1, $2)',
        [user.name, user.token], function(err, results) {
            done();
            if(err) {
              callback(err, result);
              return;
...
UserDAOPostgre.prototype.update = function(user, callback){
  ...
  this.pool.connect(function(err, client, done) {
    ...
    client.query('UPDATE git_user SET name = $1, token = $2 WHERE id = $3;', [user.name, user.token, user.id],
    function(err, result) {
        done();
        if(err) {
          callback(err, results);
...
UserDAOPostgre.prototype.get = function(user, callback){
    ...
    this.pool.connect(function(err, client, done) {
      ...
      client.query('SELECT * FROM git_user WHERE name = $1;', [user.name], function(err, result) {
        done();
        if(err) {
          callback(err, results);
          return;
